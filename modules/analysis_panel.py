...
class AnalysisPanelMixin:
    ...
    def _validate_analysis_params(self):
        if self._loading:
            messagebox.showinfo("提示", "文件仍在加载，请稍候"); return False
        if self._checking:
            messagebox.showinfo("提示", "正在分析中，请稍候"); return False
        if not self._loaded_path:
            messagebox.showwarning("提示", "请先选择 Excel 文件"); return False
        if not self.date_col_cb.get():
            messagebox.showwarning("提示", "请先选择日期列"); return False
        return True
    ...
    def _on_analysis_complete(self, detail_rows, skip, expired_count, today_count, soon_count, show_dialog):
        """分析完成后更新 UI 和状态。"""
        ...
        if self._loaded_path:
            days_ahead = self._get_analysis_params()
            days_ahead = days_ahead["days_ahead"] if days_ahead else 7
    ...
    def _run_analysis(self, show_dialog=True):
        ...
        if not self._validate_analysis_params():
            return
        ...
        log_debug(f"[ANALYSIS] 参数: days={params['days_ahead']}, col_idx={params['col_idx']}, pending_only={params['pending_only']}, date_col={self.date_col_cb.get()}")
        self._checking = True
        self._result_rows = []
        ...
        def worker():
            try:
                detail_rows, skip, expired_count, today_count, soon_count, failed_rows = \
                    self._build_detail_rows(params["days_ahead"], params["col_idx"], params["pending_only"])
                ...
                def update():
                    ...
                    finish = lambda: self._on_analysis_complete(detail_rows, skip, expired_count, today_count, soon_count, show_dialog)
    ...
    def _prepare_tree_rows(self, detail_rows):
        ...
        customer_sort_keys = {}
        for customer, product_map in customer_map.items():
            min_diff = min(r["diff"] for pr in product_map.values() for r in pr)
            customer_sort_keys[customer] = self._priority_key(min_diff)

        for customer in sorted(customer_map.keys(), key=lambda c: customer_sort_keys[c]):
            product_map = customer_map[customer]
            customer_rows = [r for pr in product_map.values() for r in pr]
            nearest = min(customer_rows, key=lambda r: self._priority_key(r["diff"]))
            ...
            product_sort_keys = {}
            for product, product_rows_list in product_map.items():
                product_sort_keys[product] = self._priority_key(min(r["diff"] for r in product_rows_list))

            for product in sorted(product_map.keys(), key=lambda p: product_sort_keys[p]):
                product_rows = product_map[product]
                nearest_p = min(product_rows, key=lambda r: self._priority_key(r["diff"]))
